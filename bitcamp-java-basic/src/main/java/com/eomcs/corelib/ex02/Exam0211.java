...
public class Exam0211 {
  public static void main(String[] args) {
    ...
    // String 리터럴이 상수풀에서 관리되는것 처럼 Integer 도 내부 메모리에 관리된다.
    ...
    //  => 다룰수 있는 숫자가 너무 많기때문에 무조건 상수풀에 만들기에는
    //      오히려 메모리 낭비가 심해지기 때문에 . -128~127 로 범위를 정함
    //  => 상수풀에 생성된 객체는 JVM 이 종료되기 전까지 유지된다. (가비지가 되지 않는다)
    //  => 그러나 Heap 에생성된 객체는 주소를 잃어버리면 가비지가 되기 때문에
    //      메모리를 좀더 효율적으로 관리할 수 있다.
