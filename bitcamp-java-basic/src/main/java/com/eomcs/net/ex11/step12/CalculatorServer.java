// 계산기 서버 만들기 - 12단계: 동시에 여러 클라이언트 요청 처리하기
...
public class CalculatorServer {
  public static void main(String[] args) {
    ...
    try (ServerSocket serverSocket = new ServerSocket(8888)) {
      ...
      while (true) {
        // 클라이언트가 접속하면 각 클라이언트의 요청을
        // main 실행에서 분리하여 별도로 실행해야 하기때문에
        ...
        thread.start();
        // start() 메서드는 main 실행과 분리하여 별도의 실행모드에서 run()을 호출한다.
        // 그 후 run()메서드의 리턴을 기다리지 않고 즉시 리턴한다.
        // 따라서 기존 클라이언트의 요청을 처리하는데 시간이 걸리더라도 다른 클라이언트는 영향을 받지 않는다.
