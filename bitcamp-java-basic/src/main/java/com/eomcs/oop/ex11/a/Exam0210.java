...
public class Exam0210 {
  ...
  static int sValue;
  static void sm() {
    // 스태틱 메서드는 인스턴스 주소를 담는 this. 라는 내장 변수가 없다.
    // 왜 ?? 클래스 이름을 사용하기 때문이다.
    // 인스턴스를 가지고 이 메서드를 호출한다고 하더라도 그 인스턴스의 주소를 저장할
    // this 라는 변수가 없기 때문에, 인스턴스를 사용할 수 없다.
  }
  ...
  void im() {
    ...
    // 인스턴스 주소 없이 호출하면 [NullPointerException 오류가] 발생한다.
    // 클래스 이름으로 바로 호출하면 [컴파일 오류]가 발생한다.
  }
  ...
  class A {
    ...
    //  => 생성자는 바깥 클래스의 인스턴스를 받는 파라미터를 갖고있다.
    // 예)
    //
    ...
    void m1() {
      ...
      // 구체적으로 어느 인스턴스 멤버를 찾을지 가르키지 않을때의 찾는 순서.
      // 1) 로컬변수를 찾는다.
      // 2) 인스턴스 변수를 찾는다.
      // 3) 스태틱 변수를 찾는다.
